...
def compute_wave_propagation(L, w, k0, v, N, dt, T):
    ...
    u_numerical[1:, :] = A_numerical[0,:]*np.cos(k**2*v*dt*n_values) - 1j*A_numerical[0,:]*np.sin(k**2*dt*v*n_values)

    u_numerical = np.fft.ifft(u_numerical, axis=1)

    u_moving = np.zeros((steps, N))
    for i, ti in enumerate(np.linspace(0, T, steps)):
        u_moving[i, :] = np.real((np.exp(-(x**2)/(4*1j*ti*v+w**2))*A*w*np.sqrt(np.pi))/(np.sqrt(np.pi)*np.sqrt(4*1j*ti*v+w**2)))
...
x, u_numerical, u_moving, steps = compute_wave_propagation(initial_L, initial_w, initial_k0, initial_v, initial_N, initial_dt, initial_T)
...
def update_output(time_step, n_clicks, L, w, k0, v, N, dt, T):
    ...
    if not ctx.triggered or ctx.triggered[0]['prop_id'].split('.')[0] == 'compute-button':
        ...
        x, u_numerical, u_moving, steps = compute_wave_propagation(L, w, k0, v, N, dt, T)
    fig = go.Figure(data=[
        go.Scatter(x=x, y=np.real(u_numerical[time_step, :]), mode='lines', name='Numerical FFT Solution'),
        go.Scatter(x=x, y=u_moving[time_step, :], mode='lines', name='Analytical Solution', line=dict(dash='dash'))
        
    ])
    fig.update_layout(
        title='Reeldel af numerisk Løsning',
        xaxis={'title': 'x'},
