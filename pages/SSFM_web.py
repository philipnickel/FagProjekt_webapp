...
def compute_wave_propagation(C1, C2, L, sigma, v, theta, N, dt, T, initial_condition, method):
    steps = int(T / dt)
    ...
    if initial_condition == 1:
        ...
        u0 = A * 1/np.cosh(A*np.sqrt(C2/(2*C1))*(x-theta)) * np.exp(1j * v/(2*C1)*x + 1j*sigma)
    ...
    u_analytical[0, :] = A * 1/np.cosh(A*np.sqrt(C2/(2*C1))*(x-theta)) * np.exp(1j * v/(2*C1)*x + 1j*sigma)
    u_analytical[1:, :] = A * 1/np.cosh(A*np.sqrt(C2/(2*C1))*(x-v*dt*n_values-theta)) * np.exp(1j * v/(2*C1)*x + 1j*((C2*A**2)/2-(v**2/(4*C1)))*dt*n_values +1j*sigma)
...
x, u_numerical, u_analytical, steps = compute_wave_propagation(initial_C1, initial_C2, initial_L, initial_sigma, initial_v, initial_theta, initial_N, initial_dt, initial_T, initial_condition, 'half')
...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Label('C1:'),
            dcc.Input(id='C1-value', type='number', value=initial_C1, step=0.1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        html.Div([
            html.Label('C2:'),
            dcc.Input(id='C2-value', type='number', value=initial_C2, step=0.1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),

        html.Div([
            html.Label('L:'),
            dcc.Input(id='L-value', type='number', value=initial_L, step=1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        
        html.Div([
            html.Label('sigma:'),
            dcc.Input(id='sigma-value', type='number', value=initial_sigma, step=1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        ...
        html.Div([
            html.Label('v:'),
            dcc.Input(id='v-value', type='number', value=initial_v, step=0.1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),

        html.Div([
            html.Label('theta:'),
            dcc.Input(id='theta-value', type='number', value=initial_theta, step=0.1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        
        html.Div([
            html.Label('N:'),
            dcc.Input(id='N-value', type='number', value=initial_N, step=1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        
        html.Div([
            html.Label('dt:'),
            dcc.Input(id='dt-value', type='number', value=initial_dt, step=0.001, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        
        html.Div([
            html.Label('T:'),
            dcc.Input(id='T-value', type='number', value=initial_T, step=1, style={'width': '110%'})
        ], className='one column', style={'margin-right': '2px'}),
        
        html.Div([
            html.Label('Part:'),
            dcc.Dropdown(id='Part', options=[{'label': 'Real', 'value': 1}, {'label': 'Imaginary', 'value': 0}], value=1, style={'width': '130%'})
        ], className='one column', style={'margin-right': '2px'}),
...
], className='row', style={'margin-bottom': '22px'}), 


    html.Div([
        html.Button('Compute', id='compute-button', n_clicks=0, style={'margin-top': '12px'})
    ], className='row', style={'margin-bottom': '22px'}),

    dcc.Graph(id='wave-animation', style={'height': '600px'}),  # This needs to be in the layout

    html.Div([
        html.Button('Play/Pause', id='play-pause-button', n_clicks=1, style={'margin-bottom': '10px'})
    ...
    dbc.Row([
        dbc.Col([
            html.Label('N:'),
            dcc.Input(id='N-value-2', type='number', value=initial_N2, step=1)
        ], width=3),
        dbc.Col([
            html.Label('Interval:'),
            dcc.Input(id='Interval-value-2', type='number', value=initial_interval, step=1)
        ], width=3),
        dbc.Col([
            html.Label('T:'),
            dcc.Input(id='T-value-2', type='number', value=initial_T2, step=1)
        ], width=3)
    ], style={'margin-bottom': '20px'}),
    ...
    html.Div([
        html.Button('Compute', id='compute-button-2', n_clicks=0, style={'margin-top': '12px'})
    ...
    html.Div([
        dcc.Loading(
            ...
            type="graph",
            children=[
                dcc.Graph(id='precision-n-plot', style={'width': '100%', 'height': 'auto'})
            ]
        )
    ], style={'margin-bottom': '20px'}),
    ...
    html.Div([
        dcc.Loading(
            ...
            type="graph",
            children=[
                dcc.Graph(id='precision-cpu-time-plot', style={'width': '100%', 'height': 'auto'})
            ]
        )
    ], style={'margin-bottom': '20px'})
])
...
def update_line_plot(time_step, n_clicks, C1, C2, L, sigma, v, theta, N, dt, T, initial_condition, Part):
    ...
    if not ctx.triggered or ctx.triggered[0]['prop_id'].split('.')[0] == 'compute-button':
        ...
        x, u_numerical, u_analytical, steps = compute_wave_propagation(C1, C2, L, sigma, v, theta, N, dt, T, initial_condition, 'half')
    
    if Part==1:
        fig = go.Figure(data=[
            go.Scatter(x=x, y=np.real(u_numerical[time_step, :]), mode='lines', name='Numerical FFT Solution'),
            go.Scatter(x=x, y=np.real(u_analytical[time_step, :]), mode='lines', name='Analytical Solution', line=dict(dash='dash'))
        ])
    elif Part==0:
        fig = go.Figure(data=[
            go.Scatter(x=x, y=np.imag(u_numerical[time_step, :]), mode='lines', name='Numerical FFT Solution'),
            go.Scatter(x=x, y=np.imag(u_analytical[time_step, :]), mode='lines', name='Analytical Solution', line=dict(dash='dash'))
        ])
...
def update_3d_plot(n_clicks,C1, C2, L, sigma, v, theta, N, dt, T,  initial_condition, Part):
    # Re-compute only if compute button is clicked
    ctx = callback_context
    if not ctx.triggered or ctx.triggered[0]['prop_id'].split('.')[0] == 'compute-button':
        x, u_numerical, u_analytical, steps = compute_wave_propagation(C1, C2, L, sigma, v, theta, N, dt, T, initial_condition, 'half')
...
def update_precision_plots(n_clicks, N, T, interval):
    ...
    u_numerical_full_solutions = [np.zeros(N_values.size, dtype=np.complex64) for _ in range(N_values.size)]
    u_numerical_half_solutions = [np.zeros(N_values.size, dtype=np.complex64) for _ in range(N_values.size)]
    u_analytical_solutions = [np.zeros(N_values.size, dtype=np.complex64) for _ in range(N_values.size)]

    for i in range(len(N_values)):
        time_start1 = time.time()
        x, u_numerical_full, u_analytical, steps = compute_wave_propagation(C1, C2, L, sigma, v, theta, N_values[i], dt, T, initial_condition, 'full')
        time_end1 = time.time()
        cpu_times_full[i] = time_end1 - time_start1
        time_start2 = time.time()
        x, u_numerical_half, u_analytical, steps = compute_wave_propagation(C1, C2, L, sigma, v, theta, N_values[i], dt, T, initial_condition, 'half')
        time_end2 = time.time()
    ...
    # create the plots
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=N_values, y=np.real(precisions_full), mode='lines', name='Full Step Method'))
    fig1.add_trace(go.Scatter(x=N_values, y=np.real(precisions_half), mode='lines', name='Half Step Method',line=dict(dash='dash')))
    fig1.update_layout(title='Precision vs N', xaxis_title='N', yaxis_title='Mean Squared Error', width=1000, height=750)

    # plot precision vs cpu time
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=cpu_times_full, y=np.real(precisions_full), mode='lines', name='Full Step Method'))
    fig2.add_trace(go.Scatter(x=cpu_times_half, y=np.real(precisions_half), mode='lines', name='Half Step Method',line=dict(dash='dash')))
    fig2.update_layout(title='Precision vs CPU Time', xaxis_title='CPU Time', yaxis_title='Mean squared error', width=1000, height=750)
...
def update_loading_state(n_clicks):
    if n_clicks:
        return [{"is_loading": True}, {"is_loading": True}]  # Show loading state if button is clicked
    else:
        return [{"is_loading": False}, {"is_loading": False}]  # Hide loading state if button is not clicked
